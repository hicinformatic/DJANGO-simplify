...
from .manager import UserManager, MethodManager
...
if conf.scheduler.enable:
    ...
    class Task(Update):
        ...
        def prepare(self):
            ...
            prepare['port']           = conf.scheduler.port
            prepare['background']     = conf.scheduler.background
            prepare['python']         = conf.scheduler.python
            prepare['directory']      = conf.directory.tasks
            prepare['extension']      = conf.scheduler.python_extension
            prepare['background_end'] = conf.scheduler.background_end
            prepare['settings_dir']   = conf.directory.settings
            self.command = conf.scheduler.command.format(**prepare)
            prepare['binary']           = conf.scheduler.binary
            prepare['script']           = conf.scheduler.can_run
            prepare['script_extension'] = conf.scheduler.can_run_extension
            prepare['timeout']          = conf.scheduler.kill_timeout
            prepare['robot']            = conf.scheduler.robot
            prepare['password']         = conf.scheduler.password
            self.local_check = conf.scheduler.local_check.format(**prepare)
