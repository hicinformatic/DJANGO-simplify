...
from .hybrids import (HybridAdminView, HybridCreateView, HybridUpdateView, HybridDetailView, HybridListView, HybridTemplateView)
from .models import (FakeModel, Method, User)
...
if conf.scheduler.enable:
    ...
    class TaskPurge(HybridTemplateView):
        ...
        def get_context_data(self, **kwargs):
            ...
            delta = timezone.now()-timedelta(days=conf.scheduler.purge_day)
            tasks = Task.objects.filter(date_update__lte=delta).order_by('-id')[:conf.scheduler.purge_number]
    ...
    class TaskMaintain(HybridTemplateView):
        ...
        def get_context_data(self, **kwargs):
            ...
            tasks = {}
            for task in conf.scheduler.maintain:
